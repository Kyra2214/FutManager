...
evidence = ['brasfoot_engine/engine/core/p1_error_contract.py', 'brasfoot_engine/engine/manager/career.py', 'brasfoot_engine/scripts/career_gateway.py', 'brasfoot_engine/tests/test_p1_error_contract.py', 'docs/p1_error_1061_1070.md']
payload = json.loads(manifest.read_text(encoding='utf-8'))
assert payload['gates']['P0_GLOBAL_GATE'] == 'OPEN'
items = payload['items']
batch = [item for item in items if 1061 <= item['item_id'] <= 1070]
...
print(json.dumps({'status': 'VALID', 'batch': '1061-1070', 'completed': 10, 'done': payload['summary']['done'], 'pending': payload['summary']['pending'], 'gates': payload['gates']}, ensure_ascii=False))
