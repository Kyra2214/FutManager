...
def ensure_p2_auditoria_registry(connection) -> None:
...
def validate_p1_auditoria(connection, item_id: int) -> dict:
    ...
    if row is None:
        raise ValueError("P2_AUDITORIA_NOT_FOUND")
...
def persist_p1_auditoria(connection, key: str, auditoria_id: int, auditoria_name: str, payload: dict, club_id: int | None = None, status: str = "ACTIVE", actor: str = "") -> dict:
    ...
    if not key.strip() or int(auditoria_id) < 0 or (club_id is not None and int(club_id) < 0) or not auditoria_name.strip() or status not in {"ACTIVE", "INACTIVE", "PENDING"} or not isinstance(payload, dict):
        raise ValueError("P2_AUDITORIA_PAYLOAD_INVALID")
...
def audit_p2_auditorias(connection) -> dict:
...
def audit_p2_auditoria_flow(connection, item_id: int, action: str, allowed: bool, reason: str, payload: dict) -> dict:
    encoded = json.dumps(payload, sort_keys=True, separators=(",", ":"))
...
# Aliases públicos do domínio 80; a implementação permanece a mesma e usa SQL/GameState.
ensure_p2_auditoria_domain80_registry = ensure_p2_auditoria_registry
validate_p2_auditoria_domain80 = validate_p1_auditoria
read_p2_auditoria_domain80s = read_p1_auditorias
protect_p2_auditoria_domain80_mutation = protect_p1_auditoria_mutation
audit_p2_auditoria_domain80s = audit_p2_auditorias
