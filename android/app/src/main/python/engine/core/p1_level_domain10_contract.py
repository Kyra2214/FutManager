...
ITEM_IDS = tuple(range(2551, 2561))
...
def ensure_p1_level_domain10_registry(connection) -> None:
    connection.executescript("""
    CREATE TABLE IF NOT EXISTS roadmap_p1_level_domain10_contracts(
    ...
    CREATE TABLE IF NOT EXISTS roadmap_p1_level_domain10_audit(
    ...
    for item_id, action in zip(ITEM_IDS, ACTIONS, strict=True):
        ...
        connection.execute("INSERT OR IGNORE INTO roadmap_p1_level_domain10_contracts VALUES(?,?,?,?,?,?,?,?,?)", (item_id, 8, "level_state", contract["payload_schema"], "CONSOLIDATED", "SQL_GAMESTATE", json.dumps(contract, sort_keys=True, separators=(",", ":")), now, now))
    connection.commit()


def validate_p1_level(connection, item_id: int) -> dict:
    row = connection.execute("SELECT * FROM roadmap_p1_level_domain10_contracts WHERE item_id=?", (item_id,)).fetchone()
    if row is None:
...
def read_p1_levels(connection) -> list[dict]:
    return [{**dict(row), "contract_json": json.loads(row["contract_json"])} for row in connection.execute("SELECT * FROM roadmap_p1_level_domain10_contracts ORDER BY item_id")]
...
def read_p1_level_domain10_state(connection, key: str | None = None) -> list[dict]:
...
def protect_p1_level_domain10_mutation(connection, item_id: int, actor: str, payload: dict) -> dict:
    valid = validate_p1_level(connection, item_id)
    allowed = actor == "AUTHORIZED_SQL_SERVICE" and valid["status"] == "VALID"
    encoded = json.dumps(payload, sort_keys=True, separators=(",", ":"))
    connection.execute("INSERT OR REPLACE INTO roadmap_p1_level_domain10_audit(item_id,action,allowed,reason,payload,created_at) VALUES(?,?,?,?,?,?)", (item_id, "PROTECT_MUTATION", int(allowed), "ALLOWED" if allowed else "SQL_SERVICE_AUTHORIZATION_REQUIRED", encoded, _now()))
    connection.commit()
    if not allowed:
        raise DomainError(DomainErrorCode.P0_MUTATION_AUTHORIZATION_REQUIRED)
...
def audit_p1_level_domain10_flow(connection, item_id: int, action: str, allowed: bool, reason: str, payload: dict) -> dict:
    encoded = json.dumps(payload, sort_keys=True, separators=(",", ":"))
    connection.execute("INSERT OR REPLACE INTO roadmap_p1_level_domain10_audit(item_id,action,allowed,reason,payload,created_at) VALUES(?,?,?,?,?,?)", (item_id, action, int(allowed), reason, encoded, _now()))
    connection.commit()
