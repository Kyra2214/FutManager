...
P1_E2E_ITEM_IDS = list(range(3911, 3921))
def ensure_p1_e2e_registry(connection):
    connection.execute('CREATE TABLE IF NOT EXISTS roadmap_p1_e2e_contracts (item_id INTEGER PRIMARY KEY, domain_id INTEGER NOT NULL, status TEXT NOT NULL, source_of_truth TEXT NOT NULL, contract_json TEXT NOT NULL)')
    for item_id in P1_E2E_ITEM_IDS:
        payload={'item_id':item_id,'domain_id':30,'integration_name':'global_integration','source_of_truth':'SQL_GAMESTATE'}
        connection.execute('INSERT OR IGNORE INTO roadmap_p1_e2e_contracts VALUES(?,?,?,?,?)',(item_id,30,'CONSOLIDATED','SQL_GAMESTATE',_json.dumps(payload,sort_keys=True)))
    connection.commit()
def audit_p1_e2e(connection):
    rows=connection.execute('SELECT * FROM roadmap_p1_e2e_contracts ORDER BY item_id').fetchall()
    return {'status':'VALID' if len(rows)==10 and {r[0] for r in rows}==set(P1_E2E_ITEM_IDS) and all(r[3]=='SQL_GAMESTATE' for r in rows) else 'INVALID','e2e_count':len(rows),'read_only':True}
