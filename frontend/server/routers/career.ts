...
export const careerRouter = router({
  ...
  worldCountries: publicProcedure
    ...
    .query(({ input }) => {
      try { return { items: listWorldCountries(input.search.trim(), input.limit) }; } catch (error) { throw toTrpcError(error); }
    }),
  ...
  parallelPreview: publicProcedure
    ...
    .query(({ input }) => {
      try { return getParallelLeaguePreview(input.selectedCountryIds, input.targetType, input.targetId); } catch (error) { throw toTrpcError(error); }
    }),
  advanceWeek: protectedProcedure
    .input(z.object({ seed: z.number().int().optional() }).optional())
    .mutation(({ input }) => {
      try { return advanceWorldWeek(input?.seed); } catch (error) { throw toTrpcError(error); }
    }),
  advanceUntilMatch: protectedProcedure
    .input(z.object({ matchId: z.number().int().positive(), seed: z.number().int().optional() }))
    .mutation(({ input }) => {
      try { return advanceUntilControlledMatch(input.matchId, input.seed); } catch (error) { throw toTrpcError(error); }
    }),
  start: protectedProcedure
    ...
    .mutation(({ input }) => {
      try { return startCareer(input); } catch (error) { throw toTrpcError(error); }
    }),
});
