...
export const staffMarketRouter = router({
  ...
  hire: protectedProcedure.input(z.object({ staffId: z.number().int().positive() })).mutation(({ input }) => {
  ...
  terminate: protectedProcedure.input(z.object({ staffId: z.number().int().positive(), waiveFee: z.boolean().optional() })).mutation(({ input }) => {
    try { return terminateStaff(input.staffId, input.waiveFee ?? false); } catch (error) { throw toTrpcError(error); }
  }),
  replace: protectedProcedure.input(z.object({ outgoingStaffId: z.number().int().positive(), incomingStaffId: z.number().int().positive() })).mutation(({ input }) => {
  ...
  registerInjury: protectedProcedure.input(z.object({ playerId: z.number().int().positive(), injuryType: z.string().min(1), severity: z.enum(["MINOR", "MODERATE", "SEVERE"]), season: z.number().int().positive(), week: z.number().int().min(1).max(53), seed: z.number().int().optional() })).mutation(({ input }) => { try { return registerInjury(input.playerId, input.injuryType, input.severity, input.season, input.week, input.seed); } catch (error) { throw toTrpcError(error); } }),
  recoverPlayers: protectedProcedure.input(z.object({ days: z.number().int().positive().default(1) })).mutation(({ input }) => { try { return recoverPlayers(input.days); } catch (error) { throw toTrpcError(error); } }),
  registerSuspension: protectedProcedure.input(z.object({ playerId: z.number().int().positive(), cards: z.number().int().min(0), redCard: z.boolean(), season: z.number().int().positive(), week: z.number().int().min(1).max(53) })).mutation(({ input }) => { try { return registerSuspension(input.playerId, input.cards, input.redCard, input.season, input.week); } catch (error) { throw toTrpcError(error); } }),
  ...
  upgradeDepartment: protectedProcedure.input(z.object({ department: z.enum(["base", "medicina", "preparacao_fisica", "analise"]) })).mutation(({ input }) => {
