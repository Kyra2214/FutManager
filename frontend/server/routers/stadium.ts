...
export const stadiumRouter = router({
  ...
  bootstrap: protectedProcedure.mutation(() => { try { return bootstrapClubStadium(); } catch (error) { throw toTrpcError(error); } }),
  ...
  upgrade: protectedProcedure.input(z.object({ component: z.enum(["arquibancada", "campo", "estrutura", "equipes"]) })).mutation(({ input }) => { try { return upgradeClubStadium(input.component); } catch (error) { throw toTrpcError(error); } }),
  ticketPrice: protectedProcedure.input(z.object({ basePrice: z.number().int().min(1).max(2000) })).mutation(({ input }) => { try { return configureClubTicketPrice(input.basePrice); } catch (error) { throw toTrpcError(error); } }),
  ...
  advanceWeek: protectedProcedure.input(z.object({ seed: z.number().int().optional() }).optional()).mutation(({ input }) => { try { return advanceWorldWeek(input?.seed); } catch (error) { throw toTrpcError(error); } }),
});
