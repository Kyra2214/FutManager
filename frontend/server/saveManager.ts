...
function ensureRoot() {
  mkdirSync(SAVE_ROOT, { recursive: true });
}

function validId(id: string) {
  return /^[a-z0-9][a-z0-9_-]{0,31}$/i.test(id) && id !== "default";
}
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        currentClubId: row.current_club_id == null ? null : Number(row.current_club_id),
...
export function listSaveSlots() {
  ...
  return [{ id: DEFAULT_SAVE_SLOT, name: "Jogo principal", createdAt: null, updatedAt: null, databasePath: DEFAULT_DATABASE_PATH, career: careerSummary(DEFAULT_DATABASE_PATH), isDefault: true },
    ...items.map(item => ({ ...item, databasePath: slotPath(item.id), career: careerSummary(slotPath(item.id)), isDefault: false }))];
}

export function resolveSaveDatabasePath(id: string | undefined | null) {
  const slot = id || DEFAULT_SAVE_SLOT;
  const known = slot === DEFAULT_SAVE_SLOT || readIndex().some(item => item.id === slot);
  if (!known) throw new Error("SAVE_SLOT_NOT_FOUND");
...
export function createSaveSlot(name: string, sourceSlot = DEFAULT_SAVE_SLOT) {
  ...
  const items = readIndex();
  if (items.length >= MAX_SLOTS) throw new Error("SAVE_SLOT_LIMIT_REACHED");
  const baseId = cleanName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 24) || "save";
  let id = baseId;
  let suffix = 2;
  while (items.some(item => item.id === id) || id === DEFAULT_SAVE_SLOT) id = `${baseId}-${suffix++}`;
  const sourcePath = resolveSaveDatabasePath(sourceSlot);
  const targetPath = slotPath(id);
  ensureRoot();
  if (existsSync(sourcePath)) copyFileSync(sourcePath, targetPath);
  else writeFileSync(targetPath, "");
  const timestamp = now();
  const meta: SaveMeta = { id, name: cleanName, createdAt: timestamp, updatedAt: timestamp };
  writeIndex([...items, meta]);
  return { ...meta, databasePath: targetPath, career: careerSummary(targetPath), isDefault: false };
}

export function deleteSaveSlot(id: string) {
  if (id === DEFAULT_SAVE_SLOT) throw new Error("DEFAULT_SAVE_CANNOT_DELETE");
  const items = readIndex();
  if (!items.some(item => item.id === id)) throw new Error("SAVE_SLOT_NOT_FOUND");
...
export function renameSaveSlot(id: string, name: string) {
  ...
  if (index < 0) throw new Error("SAVE_SLOT_NOT_FOUND");
  items[index] = { ...items[index], name: cleanName, updatedAt: now() };
  writeIndex(items);
  return items[index];
}

export function touchSaveSlot(id: string) {
  if (id === DEFAULT_SAVE_SLOT) return;
  const items = readIndex();
  const index = items.findIndex(item => item.id === id);
  if (index < 0) return;
  items[index] = { ...items[index], updatedAt: now() };
  writeIndex(items);
}
