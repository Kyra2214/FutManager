...
export function InteractiveMatchCenter({ match, players, onAfterPlay }: { match: Match; players: Player[]; onAfterPlay?: () => void }) {
  ...
  const start = () => {
    if (!match.matchId) {
      toast.error("Esta partida ainda não possui um identificador oficial. Atualize o calendário antes de iniciar.");
      return;
    }
    setEvents([]);
    setStep(0);
  ...
  return <section className={`interactive-match-center ${isLive ? "is-live" : ""} ${isPaused ? "is-paused" : ""}`} aria-labelledby="interactive-match-title">
    ...
    <div className="interactive-actions">{!isLive && !playMatch.isPending && <button className="primary-action" type="button" onPointerDown={start} data-testid="start-match-button"><Play size={16} /> Iniciar partida</button>}{isLive && !isPaused && <button className="outline-action" type="button" onClick={() => setIsPaused(true)}><Pause size={16} /> Pausar jogo</button>}{isPaused && <button className="primary-action" type="button" onClick={continueMatch}><Play size={16} /> Retomar jogo</button>}{playMatch.isPending && <span className="match-saving"><RotateCcw size={15} className="spin" /> Registrando resultado oficial…</span>}</div>
    <div className="interactive-grid"><article className="live-feed"><div className="section-heading compact"><div><span className="eyebrow">LANCES DA PARTIDA</span><h3>O jogo acontece aqui</h3></div><Flag size={17} /></div><div className="live-feed-list">{events.length ? events.map((event) => { const Icon = eventIcon(event.tone); return <div className={`live-event tone-${event.tone}`} key={`${event.minute}-${event.title}`}><span className="event-minute">{event.minute}'</span><span className="event-icon"><Icon size={15} /></span><div><b>{event.title}</b><p>{event.detail}</p></div></div>; }) : <div className="live-feed-empty"><Swords size={20} /><p>Os lances aparecerão conforme a partida avançar.</p></div>}</div></article><aside className="tactical-console"><div className="section-heading compact"><div><span className="eyebrow">CONSOLE TÁTICO</span><h3>Comandos do manager</h3></div><Target size={17} /></div><div className="tactic-field"><label>MENTALIDADE<select value={tactics.mentality} onChange={(event) => setTactic("mentality", event.target.value)} disabled={isLive && !isPaused}><option value="DEFENSIVE">Defensivo</option><option value="NEUTRAL">Neutro</option><option value="OFFENSIVE">Ofensivo</option></select></label><label>FORMAÇÃO<select value={redCardResponse.formation} onChange={(event) => setRedCardResponse((current) => ({ ...current, formation: event.target.value }))} disabled={isLive && !isPaused}><option>4-4-2</option><option>4-3-3</option><option>3-5-2</option><option>5-3-2</option></select></label><label>ATAQUE<select value={tactics.attackLane} onChange={(event) => setTactic("attackLane", event.target.value)} disabled={isLive && !isPaused}><option value="MIXED">Misto</option><option value="WINGS">Pelas laterais</option><option value="CENTER">Pelo meio</option></select></label><label>PASSES<select value={tactics.passing} onChange={(event) => setTactic("passing", event.target.value)} disabled={isLive && !isPaused}><option value="SHORT">Curtos</option><option value="LONG">Longos</option><option value="SIDEWAYS">De lado</option></select></label><label>PRESSÃO<select value={tactics.pressure} onChange={(event) => setTactic("pressure", event.target.value)} disabled={isLive && !isPaused}><option value="LOW">Bloco baixo</option><option value="MEDIUM">Equilibrada</option><option value="HIGH">Alta</option><option value="LINE">Marcação em linha</option></select></label><label className="tactic-check"><input type="checkbox" checked={tactics.crossing} onChange={(event) => setTactic("crossing", event.target.checked)} disabled={isLive && !isPaused} /> Cruzar mais bolas</label></div></aside></div>
