  ...
  return <main className="career-start-shell">
    ...
    <section className="career-start-panel">
      ...
      <section className="career-world-config" aria-labelledby="world-config-title">
        ...
        {worldCountriesQuery.isLoading ? <div className="career-target-loading">Carregando ligas oficiais…</div> : worldCountriesQuery.isError ? <div className="career-target-loading error">Não foi possível consultar os países do motor.</div> : <div className="career-world-countries">{selectedCountries.map((country) => <button type="button" key={country.countryId} className={`career-world-country ${selectedCountryIds.includes(country.countryId) ? "selected" : ""} ${country.supported === false ? "unavailable" : ""}`} disabled={country.supported === false} onClick={() => toggleCountry(country.countryId)}><span><b>{country.name}</b><small>{country.membershipStatus === "OFFICIAL" ? `${country.code || "PAÍS"} · ${country.firstDivisionClubCount || 0} clube(s) oficiais` : `${country.code || "PAÍS"} · ${country.eligibleClubCount || country.clubCount} clube(s) válidos no SQL`}
</small></span>{selectedCountryIds.includes(country.countryId) && <Check size={15} />}</button>)}</div>}
        <div className="career-world-summary" aria-live="polite"><span>{selectedCountryIds.length} liga(s) selecionada(s)</span><b>{selectedCountryIds.length ? selectedCountries.filter((country) => selectedCountryIds.includes(country.countryId)).map((country) => country.name).join(" + ") : "Escolha pelo menos uma liga"}</b><small>{selectedCountryIds.length === 1 ? "A competição nacional permanece intacta; o clube da 1ª divisão pode iniciar na 4ª." : selectedCountryIds.length > 1 ? `${selectedFirstDivisionClubCount} clube(s) oficiais + ${Math.max(0, 80 - selectedFirstDivisionClubCount)} complemento(s) do SQL completam 80 (20 por divisão)` : "Os clubes nacionais continuam em suas ligas originais"}
</small></div>
