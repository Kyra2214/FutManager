...
export default function CareerStart({ onStarted, onContinue }: { onStarted: () => void; onContinue?: () => void }) {
  ...
  return <main className="career-start-shell">
    <section className="career-start-hero">
      ...
      <div className="career-start-kicker">NOVA CARREIRA · PRIMEIRO APITO</div>
      <h1>Escolha <em>o seu lugar</em><br />no futebol.</h1>
      <p>Monte seu universo, escolha um clube ou seleção e comece a construir sua história no futebol.</p>
      <div className="career-start-steps"><span className="active">01 Você</span><span className={selectedCountryIds.length ? "active" : ""}>02 Universo</span><span>03 Clube</span><span>04 Apito</span></div>
    </section>
    <section className="career-start-panel">
      {activeSave && <div className="active-save-card"><div><span className="eyebrow">CARREIRA EM ANDAMENTO</span><strong>{activeSave.careerName}</strong><small>{activeSave.targetName} · {activeSave.targetType === "club" ? "Clube" : "Seleção"}</small><small>{activeSave.selectedCountries?.map((country) => country.name).join(" + ") || "Universo padrão"} · carreira pronta para continuar</small></div><button className="outline-action" type="button" onClick={onContinue}>Continuar save <ArrowRight size={16} /></button></div>}
      <header><span className="eyebrow">PRIMEIRO APITO</span><h2>Onde começa sua história?</h2><p>Escolha seu perfil, o universo de ligas e quem você vai comandar.</p></header>
      ...
      <div className="career-targets" aria-live="polite">{catalogQuery.isLoading && <div className="career-target-loading">Carregando entidades oficiais…</div>}{catalogQuery.isError && <div className="career-target-loading error">Não foi possível carregar os clubes agora.</div>}{!catalogQuery.isLoading && !catalogQuery.isError && (catalogQuery.data ?? []).filter((item) => targetType !== "club" || Boolean(item.name?.trim())).length === 0 && <div className="career-target-loading">Nenhuma entidade oficial foi encontrada para essa busca.</div>}
{catalogQuery.data?.filter((item) => targetType !== "club" || Boolean(item.name?.trim())).map((item) => { const presentation = { label: item.assetKind === "kit" ? "Camisa oficial" : item.assetUrl ? "Escudo oficial" : "Escudo pendente" };
 const selected = item.entityId === selectedId; return <button type="button" key={item.entityId} className={`career-target-card ${selected ? "selected" : ""}`} onClick={() => setSelectedId(item.entityId)}><span className="career-target-asset">{item.assetUrl ? <img src={item.assetUrl} alt={item.assetKind === "kit" ? `Camisa de ${item.name}` : `Escudo de ${item.name}`} /> : <span aria-label="Escudo ainda não mapeado">⚽</span>}</span><span className="career-target-copy"><b>{item.name}</b><small>{presentation.label}</small></span>{selected && <span className="career-selected"><Check size={15} /></span>}</button>; })}</div>
