...
type WorldCountry = { countryId: number; name: string; code: string | null; clubCount: number };
  ...
  return <main className="career-start-shell">
    ...
    <section className="career-start-panel">
      ...
      <section className="career-world-config" aria-labelledby="world-config-title">
        ...
        {worldCountriesQuery.isLoading ? <div className="career-target-loading">Carregando ligas oficiais…</div> : worldCountriesQuery.isError ? <div className="career-target-loading error">Não foi possível consultar os países do motor.</div> : <div className="career-world-countries">{selectedCountries.map((country) => <button type="button" key={country.countryId} className={`career-world-country ${selectedCountryIds.includes(country.countryId) ? "selected" : ""}`} onClick={() => toggleCountry(country.countryId)}><span><b>{country.name}</b><small>{country.code || "PAÍS"} · {country.clubCount} clube(s)</small></span>{selectedCountryIds.includes(country.countryId) && <Check size={15} />}</button>)}</div>}
        <div className="career-world-summary" aria-live="polite"><span>{selectedCountryIds.length} liga(s) selecionada(s)</span><b>{selectedCountryIds.length ? selectedCountries.filter((country) => selectedCountryIds.includes(country.countryId)).map((country) => country.name).join(" + ") : "Escolha pelo menos uma liga"}</b></div>
      </section>
