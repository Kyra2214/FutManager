...
export function MatchesPage({ initialView = "competicoes" }: { initialView?: "competicoes" | "tabela" | "calendario" | "resultados" }) {
  ...
  return <>
    ...
    <section className={`matchday-simulation ${matchSimulationOpen ? "is-open" : ""}`} aria-labelledby="matchday-title"><div className="matchday-simulation-head"><div><span className="eyebrow">CENTRAL DO DIA</span><h2 id="matchday-title">Acompanhar o próximo jogo</h2><p>{nextMatch ? `${nextMatch.homeClub.name} × ${nextMatch.awayClub.name} · rodada ${nextMatch.round ?? "—"}` : "Aguardando uma partida real no calendário."}</p></div><button className="primary-action compact" type="button" disabled={!nextMatch} onClick={() => setMatchSimulationOpen((open) => !open)}>{matchSimulationOpen ? "Fechar simulação" : "Abrir simulação"}<ArrowUpRight size={15} /></button></div>{matchSimulationOpen && nextMatch && <div className="matchday-simulation-body"><div className="matchday-progress"><span>FASE {simulationStep + 1} / {simulationStages.length}</span><strong>{simulationStages[simulationStep]}</strong><div className="matchday-progress-bar"><i style={{ width: `${((simulationStep + 1) / simulationStages.length) * 100}%` }} /></div></div><div className="matchday-pitch"><div><EntityAsset className="crest-placeholder" entityType="team" entityId={nextMatch.homeClub.clubId} entityName={nextMatch.homeClub.name} /><b>{nextMatch.homeClub.name}</b></div><span>VS</span><div><EntityAsset className="crest-placeholder away" entityType="team" entityId={nextMatch.awayClub.clubId} entityName={nextMatch.awayClub.name} /><b>{nextMatch.awayClub.name}</b></div></div><div className="matchday-events"><p><span className="status-pip" />A simulação acompanha somente o estado do jogo. O placar e os eventos aparecem quando forem persistidos pelo motor.</p><p>Próximo passo: {simulationStages[Math.min(simulationStep + 1, simulationStages.length - 1)]}.</p></div></div>}</section>
