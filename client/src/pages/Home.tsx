...
export function MatchesPage({ initialView = "competicoes" }: { initialView?: "competicoes" | "tabela" | "calendario" | "resultados" }) {
  ...
  return <>
    ...
    <section className="matchday-command" aria-labelledby="matchday-title"><div className="matchday-simulation-head"><div><span className="eyebrow">CENTRAL DO DIA</span><h2 id="matchday-title">Assuma o controle da próxima partida</h2><p>{nextMatch ? `${nextMatch.homeClub.name} × ${nextMatch.awayClub.name} · rodada ${nextMatch.round ?? "—"}` : "Aguardando uma partida real no calendário."}</p></div><button className="outline-action compact" type="button" onClick={() => advanceWeek.mutate({})} disabled={advanceWeek.isPending}>{advanceWeek.isPending ? "Avançando…" : "Avançar semana"}</button></div>{nextMatch ? <InteractiveMatchCenter match={nextMatch} players={workspaceQuery.data?.squad.players ?? []} /> : <div className="matchday-empty">Nenhuma partida controlada está disponível para iniciar.</div>}</section>
