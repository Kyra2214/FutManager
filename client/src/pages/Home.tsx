...
function Header({ section, onMenu }: { section: AppSection; onMenu: () => void }) {
  ...
  return (
    <header className="topbar">
      ...
      <div className="top-actions"><span className="data-status"><span className="status-pip" /> CARREIRA ATIVA</span><div className="alerts-control"><button className="icon-btn" onClick={() => setAlertsOpen((open) => !open)} aria-label="Alertas" aria-expanded={alertsOpen}><Bell size={18} />{unread > 0 && <i>{unread > 9 ? "9+" : unread}</i>}</button>{alertsOpen && <section className="alerts-popover" aria-label="Alertas persistidos"><div className="alerts-popover-head"><div><span className="eyebrow">EVENTOS DO CLUBE</span><h2>Alertas</h2></div><button onClick={() => setAlertsOpen(false)} aria-label="Fechar alertas"><X size={16} /></button></div>{eventsQuery.isLoading ? <p className="alerts-state">Consultando os eventos persistidos…</p> : eventsQuery.error ? <p className="alerts-state">Os alertas não puderam ser consultados.</p> : alerts.length ? <div className="alerts-list">{alerts.map((event) => <button key={event.event_id} className={`alert-row ${event.is_read ? "is-read" : "is-open"}`} onClick={() => { if (!event.is_read) markRead.mutate({ eventId: event.event_id }); }}><span className={`alert-severity severity-${event.severity.toLowerCase()}`} /><span><b>{event.title}</b><small>{event.description ?? event.type} · {event.event_date}</small></span></button>)}</div> : <p className="alerts-state">Nenhum alerta foi persistido para este clube.</p>}</section>}</div><button className="avatar" onClick={() => toast("Perfil do manager será conectado na próxima etapa.")} aria-label="Perfil">FM</button></div>
    </header>
  );
}
...
export function MatchesPage({ initialView = "competicoes" }: { initialView?: "competicoes" | "tabela" | "calendario" | "resultados" }) {
  ...
  const playMatch = trpc.matches.playControlled.useMutation({
    onSuccess: async (result) => {
      ...
      toast.success(`Partida registrada: ${result.home_goals} × ${result.away_goals}.`);
    },
    onError: () => toast.error("Não foi possível registrar a partida do clube.")
  });
  ...
  return <>
    ...
    <section className={`matchday-simulation ${matchSimulationOpen ? "is-open" : ""}`} aria-labelledby="matchday-title"><div className="matchday-simulation-head"><div><span className="eyebrow">CENTRAL DO DIA</span><h2 id="matchday-title">Preparar a próxima partida</h2><p>{nextMatch ? `${nextMatch.homeClub.name} × ${nextMatch.awayClub.name} · rodada ${nextMatch.round ?? "—"}` : "Aguardando uma partida real no calendário."}</p></div><div className="matchday-actions"><button className="primary-action compact" type="button" disabled={!nextMatch} onClick={() => setMatchSimulationOpen((open) => !open)}>{matchSimulationOpen ? "Fechar detalhes" : "Abrir detalhes"}<ArrowUpRight size={15} /></button><button className="outline-action compact" type="button" onClick={() => advanceWeek.mutate({})} disabled={advanceWeek.isPending}>{advanceWeek.isPending ? "Avançando…" : "Avançar semana"}</button><button className="primary-action compact" type="button" onClick={() => nextMatch?.matchId && playMatch.mutate({ matchId: nextMatch.matchId })} disabled={!nextMatch?.matchId || playMatch.isPending}>{playMatch.isPending ? "Registrando…" : "Jogar partida"}</button></div></div>{matchSimulationOpen && nextMatch && <div className="matchday-simulation-body"><div className="matchday-pitch"><div><EntityAsset className="crest-placeholder" entityType="team" entityId={nextMatch.homeClub.clubId} entityName={nextMatch.homeClub.name} /><b>{nextMatch.homeClub.name}</b></div><span>VS</span><div><EntityAsset className="crest-placeholder away" entityType="team" entityId={nextMatch.awayClub.clubId} entityName={nextMatch.awayClub.name} /><b>{nextMatch.awayClub.name}</b></div></div><div className="matchday-events"><p><span className="status-pip" />A partida do seu clube não é processada pelo avanço automático. Quando estiver pronto, use “Jogar partida” para registrar o resultado oficial.</p>{matchdayEventsQuery.isLoading ? <p>Atualizando os acontecimentos recentes…</p> : matchdayEvents.length ? <div className="matchday-event-feed">{matchdayEvents.slice(0, 3).map((event) => <p key={event.event_id}><span className="status-pip" />{event.title}{event.description ? ` · ${event.description}` : ""}</p>)}</div> : <p>Nenhum acontecimento recente para esta carreira.</p>}</div></div>}</section>
