...
function StructurePage({ section, onSectionChange }: { section: AppSection; onSectionChange: (section: AppSection) => void }) {
  ...
  const structureRows = isStadium
    ...
    : isCt
      ? workspace?.staff.members.length
        ? workspace.staff.members.map((member) => ({ label: `${member.name} · ${member.role}`, value: `nível ${member.level} · exp. ${member.experience} · rep. ${member.reputation}${member.specialization ? ` · ${member.specialization}` : ""}` }))
        : [{ label: "Comissão técnica", value: "nenhum profissional persistido" }, { label: "Médicos", value: "nenhum médico persistido" }, { label: "Auxiliares", value: "nenhum auxiliar persistido" }, { label: "Departamentos", value: workspace?.staff.departments.length ? `${workspace.staff.departments.length} persistido(s)` : "nenhum departamento persistido" }]
