...
export function StructurePage({ section, onSectionChange }: { section: AppSection; onSectionChange: (section: AppSection) => void }) {
  ...
  const statsFilterInput = useMemo(() => playerFilter.trim() ? { matchIds: undefined } : undefined, [playerFilter]);
  const playerStatsQuery = trpc.matches.playerStats.useQuery(statsFilterInput, { retry: 1, enabled: isTeam });
  const workspace = workspaceQuery.data;
