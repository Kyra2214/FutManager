...
export function MatchesPage({ initialView = "competicoes" }: { initialView?: "competicoes" | "tabela" | "calendario" | "resultados" }) {
  ...
  const simulationStages = ["Preparação do jogo", "Escalações confirmadas", "A bola vai rolar", "Partida em andamento"];
  useEffect(() => {
    if (!matchSimulationOpen) { setSimulationStep(0); return; }
    const timer = window.setInterval(() => setSimulationStep((step) => Math.min(step + 1, simulationStages.length - 1)), 2200);
  ...
  return <>
    <section className="page-intro match-intro"><div><span className="eyebrow">SEU CLUBE / CICLO ESPORTIVO</span><h1>Dia do jogo</h1><p>Prepare a rodada, acompanhe o calendário e entre na simulação quando o próximo compromisso estiver confirmado.</p></div><div className="match-score-mark"><span>JOGOS NA TEMPORADA</span><strong>{dashboardQuery.isLoading ? "—" : totalMatches}</strong><small>{selectedCompetition?.seasonYear ? `temporada ${selectedCompetition.seasonYear}` : "estado atual"}</small></div></section>
    ...
    <section className={`matchday-simulation ${matchSimulationOpen ? "is-open" : ""}`} aria-labelledby="matchday-title"><div className="matchday-simulation-head"><div><span className="eyebrow">CENTRAL DO DIA</span><h2 id="matchday-title">Acompanhar o próximo jogo</h2><p>{nextMatch ? `${nextMatch.homeClub.name} × ${nextMatch.awayClub.name} · rodada ${nextMatch.round ?? "—"}` : "Aguardando uma partida real no calendário."}</p></div><button className="primary-action compact" type="button" disabled={!nextMatch} onClick={() => setMatchSimulationOpen((open) => !open)}>{matchSimulationOpen ? "Fechar simulação" : "Abrir simulação"}<ArrowUpRight size={15} /></button></div>{matchSimulationOpen && nextMatch && <div className="matchday-simulation-body"><div className="matchday-progress"><span>FASE {simulationStep + 1} / {simulationStages.length}</span><strong>{simulationStages[simulationStep]}</strong><div className="matchday-progress-bar"><i style={{ width: `${((simulationStep + 1) / simulationStages.length) * 100}%` }} /></div></div><div className="matchday-pitch"><div><EntityAsset className="crest-placeholder" entityType="team" entityId={nextMatch.homeClub.clubId} entityName={nextMatch.homeClub.name} /><b>{nextMatch.homeClub.name}</b></div><span>VS</span><div><EntityAsset className="crest-placeholder away" entityType="team" entityId={nextMatch.awayClub.clubId} entityName={nextMatch.awayClub.name} /><b>{nextMatch.awayClub.name}</b></div></div><div className="matchday-events"><p><span className="status-pip" />A simulação acompanha somente o estado do jogo. O placar aparece quando o resultado for confirmado.</p><p>Próximo passo: {simulationStages[Math.min(simulationStep + 1, simulationStages.length - 1)]}.</p>{matchdayEventsQuery.isLoading ? <p>Atualizando os acontecimentos recentes…</p> : matchdayEvents.length ? <div className="matchday-event-feed">{matchdayEvents.slice(0, 3).map((event) => <p key={event.event_id}><span className="status-pip" />{event.title}{event.description ? ` · ${event.description}` : ""}</p>)}</div> : <p>Nenhum acontecimento recente para esta carreira.</p>}</div></div>}</section>
