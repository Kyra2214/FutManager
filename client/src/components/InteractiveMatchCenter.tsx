...
export function InteractiveMatchCenter({ match, players, onAfterPlay }: { match: Match; players: Player[]; onAfterPlay?: () => void }) {
  ...
  const [localPlayPending, setLocalPlayPending] = useState(false);
  ...
  useEffect(() => {
    if (!isLive || isPaused || (isNative ? localPlayPending : playMatch.isPending) || step >= timeline.length - 1) return;
  ...
  useEffect(() => {
    ...
    if (current.final && match.matchId && !(isNative ? localPlayPending : playMatch.isPending)) {
      setIsLive(false);
      const decisions = { tactics, substitutions, penalty_taker_id: penaltyTakerId, red_card_response: redCardResponse };
      if (isNative) {
        setLocalPlayPending(true);
        localDomain.playControlledMatch(match.matchId, decisions).then((result) => {
          const homeGoals = Number(result.home_goals ?? 0);
          const awayGoals = Number(result.away_goals ?? 0);
          setEvents((currentEvents) => [...currentEvents, { minute: 90, title: "Resultado oficial", detail: `${homeGoals} × ${awayGoals} registrado na carreira.`, tone: "neutral" }]);
          toast.success(`Resultado oficial: ${homeGoals} × ${awayGoals}.`);
          onAfterPlay?.();
        }).catch(() => {
          setIsLive(false);
          setIsPaused(false);
          toast.error("A partida não pôde ser registrada no dispositivo. Nenhum resultado foi alterado.");
        }).finally(() => setLocalPlayPending(false));
      } else {
        playMatch.mutate({ matchId: match.matchId, decisions });
      }
    }
  }, [isLive, step, isNative, localPlayPending]);
